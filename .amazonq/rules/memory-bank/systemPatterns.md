...
├── kiro.sh                 # Launches kiro-cli with jobbet agent config
├── deploy.env              # Resource names (gitignored, generated by setup script)
├── .kiro/agents/jobbet.json # Agent config: whitelisted tools & shell commands
├── .amazonq/rules/
│   ├── development-workflow.md  # Checklist for all changes
...
- Development workflow rule: `.amazonq/rules/development-workflow.md` enforces a checklist (requirements → design → code → tests → deploy → integration test)
